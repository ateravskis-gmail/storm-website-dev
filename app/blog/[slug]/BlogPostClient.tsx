    ...
    <>
      {content.map((item, index) => {
        if (item.type === 'paragraph') {
          ...
          if ((item as { formatted?: boolean }).formatted) {
    ...
    <>
      {content.map((item, index) => {
        ...
        if (item.type === 'image' && item.src) {
          const isLarge = (item as { size?: string }).size === 'large'
          return (
            <div
              key={index}
...
const textPostContent: Record<string, string> = {
  'digital-transformation-stormwater-industry': `Stormwater Compliance Was Built on the Wrong Foundation — And It's Time We Fix It
...
In this post, I want to walk you through how we got here — and why we're standing on the edge of a necessary reinvention.`,
  'ai-slop-armageddon': `AI has never moved faster than it is right now. Every month brings a breakthrough, a new model, or a new capability that seemed impossible just a year ago. Technology is improving exponentially, not linearly — and if you've been watching closely, you can feel the acceleration.
...
export default function BlogPostClient({ post }: BlogPostClientProps) {
  ...
  const textContent = textPostContent[slug]
      ...
      <article className="pt-32 pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <motion.header
            ...
            className="mb-12"
          >
            <div className="text-sm text-gray-500 mb-4">{formatPostDate(post.datePublished)}</div>
            ...
            <div className="bg-white rounded-2xl p-8 md:p-12 shadow-lg">
              ...
              <div className="mt-12 pt-8 border-t border-gray-200">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900 text-lg">Andrew Teravskis</div>
                    <div className="text-gray-600 text-sm">Founder and CEO, Storm</div>
                  </div>
                </div>
              </div>
            </div>
